...
const LoadingSpinner = ({ current, total }: { current?: number, total?: number }) => (
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex flex-col items-center justify-center z-50 animate-fadeIn">
        <div className="bg-white dark:bg-slate-800 p-8 rounded-2xl shadow-2xl flex flex-col items-center">
            ...
            <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-2">Analisando Documento{total && total > 1 ? `s (${current}/${total})` : ''}...</h3>
...
const App: React.FC = () => {
    ...
    const loadFromHistory = (item: HistoryItem) => {
        ...
        if (fullResult) {
            ...
            if (item.fileName === 'Rascunho Manual') {
                ...
                setSelectedFiles([{ file: { name: item.fileName } as File, base64: '', mimeType: '' }]);
            }
            setError(null);
    ...
    const handleStartAnalysis = async () => {
        ...
        if (selectedFiles.length === 0) { setError("Selecione um arquivo."); return; }
        setIsLoading(true); setError(null);
        setAnalysisProgress({ current: 1, total: selectedFiles.length });
        try {
            ...
            for (let i = 0; i < selectedFiles.length; i++) {
                ...
                const result = await analyzeDocument(fileItem.base64, mime);
                const newItem = saveToHistory(result, headerData, fileItem.file.name);
            ...
            if (newItems.length === 1) {
                const fullRes = getFullResult(newItems[0]);
                if (fullRes) setAnalysisResult(fullRes);
                setComparisonResult(null);
                setConsolidationResult(null);
            } else if (newItems.length > 1) {
                handleConsolidation(newItems);
            }
        } catch (err: any) {
            console.error(err);
    ...
    const isReady = !isLoading && selectedFiles.length > 0 && selectedFiles.every(f => f.base64.length > 0);
    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col transition-colors duration-300">
            ...
            <main className="flex-1 max-w-7xl w-full mx-auto px-6 py-8">
                {isLoading && <LoadingSpinner current={analysisProgress.current} total={analysisProgress.total} />}
