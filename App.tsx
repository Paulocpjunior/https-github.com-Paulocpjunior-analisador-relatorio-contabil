...
const App: React.FC = () => {
  ...
  const loadFromHistory = (item: HistoryItem) => {
      ...
      if (fullResult) {
          ...
          setSelectedFile({ file: { name: item.fileName } as File, base64: '' });
          setError(null);
  ...
  const isReady = !isLoading && selectedFile && selectedFile.base64;

  return (
    <div className="min-h-screen bg-slate-100 dark:bg-slate-900 pb-12 transition-colors duration-300">
      <AnalysisHistory isOpen={isHistoryOpen} onClose={() => setIsHistoryOpen(false)} history={history} onSelect={loadFromHistory} onClear={() => setHistory([])} />
      ...
      <main className="max-w-5xl mx-auto px-4 py-8">
        ...
        {!analysisResult && (
           <div className={isLoading ? 'opacity-50 blur-sm pointer-events-none' : ''}>
              ...
              <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
                  ...
                  <FileUploader onFileSelected={(f, b) => { setSelectedFile({file: f, base64: b}); setError(null); }} isLoading={isLoading} selectedFileName={selectedFile?.file.name} />
                  ...
                  <button 
                    ...
                    className={`w-full py-4 rounded-lg font-bold text-lg shadow-lg transition-all
                        ${isReady ? 'bg-green-600 hover:bg-green-700 text-white transform hover:scale-[1.01]' : 'bg-slate-300 text-slate-500 cursor-not-allowed'}`}
                  >
                     {isLoading ? 'Processando...' : 'Iniciar Análise com IA'}
                  </button>
              </div>
           </div>
        )}
        {analysisResult && !isLoading && (
            <div className="animate-fadeIn">
                {/* Deleted the header logic here because it is now INSIDE AnalysisViewer to handle layout better */}
