...
const AnalysisHistory: React.FC<Props> = ({ isOpen, onClose, history, onSelect, onClear }) => {
  ...
  return (
    <div className={`fixed inset-0 z-50 overflow-hidden transition-all duration-300 ${isOpen ? 'pointer-events-auto' : 'pointer-events-none'}`}>
      ...
      {/* Sidebar */}
      <div className={`absolute inset-y-0 right-0 max-w-md w-full bg-white shadow-2xl flex flex-col transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        {/* Header */}
        <div className="px-6 py-4 bg-slate-100 border-b border-slate-200 flex items-center justify-between">
          ...
          <button onClick={onClose} className="text-slate-500 hover:text-slate-700 p-2 hover:bg-slate-200 rounded-full transition-colors">
            ...
            <ul className="space-y-3">
              {history.map((item) => (
                <li 
                  ...
                  className="bg-white p-4 rounded-lg border border-slate-200 shadow-sm hover:border-blue-400 hover:shadow-md cursor-pointer transition-all group"
                >
        ...
        {history.length > 0 && (
          <div className="p-4 bg-slate-100 border-t border-slate-200">
            <button 
              onClick={onClear}
              className="w-full py-2 px-4 bg-white border border-red-200 text-red-600 hover:bg-red-50 rounded-md text-sm font-medium transition-colors flex items-center justify-center"
            >
