    ...
    <div className="mb-2">
      <label
        ...
        className={`relative flex flex-col items-center justify-center w-full h-64 border-2 border-dashed rounded-xl cursor-pointer transition-all
          ${dragActive 
            ...
            : selectedFileName 
                ? 'border-green-50 bg-green-50 dark:bg-green-900/20 dark:border-green-800' 
                : 'border-slate-300 bg-white hover:bg-slate-50 dark:bg-slate-800 dark:border-slate-600 dark:hover:bg-slate-750'
          }
      ...
      >
        <div className="flex flex-col items-center justify-center pt-5 pb-6 text-center p-4">
          {selectedFileName ? (
             <>
               <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-12 h-12 mb-4 text-green-600 dark:text-green-500">
                 <path fillRule="evenodd" d="M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12zm13.36-1.814a.75.75 0 10-1.22-.872l-3.236 4.53L9.53 12.22a.75.75 0 00-1.06 1.06l2.25 2.25a.75.75 0 001.14-.094l3.75-5.25z" clipRule="evenodd" />
               </svg>
               <p className="text-lg font-semibold text-green-800 dark:text-green-300 mb-1">Arquivo Selecionado!</p>
               <p className="text-base font-medium text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-700 px-4 py-2 rounded-md border border-slate-200 dark:border-slate-600 shadow-sm max-w-xs truncate">
                 {selectedFileName}
               </p>
               <p className="text-sm text-slate-500 dark:text-slate-400 mt-4">Clique ou arraste para trocar de arquivo</p>
             </>
          ) : (
             <>
