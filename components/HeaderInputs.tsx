...
const HeaderInputs: React.FC<Props> = ({ data, onChange, onSave, disabled }) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 mb-6 transition-colors relative">
      ...
      {/* Tipo de Análise Toggle */}
      <div className="flex bg-slate-100 dark:bg-slate-900/50 p-1 rounded-lg mb-6 w-fit">
        <button
          type="button"
          onClick={() => onChange({ ...data, isGroupAnalysis: false })}
          className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${!data.isGroupAnalysis
        ...
        <button
          type="button"
          onClick={() => onChange({ ...data, isGroupAnalysis: true })}
          className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${data.isGroupAnalysis
              ? 'bg-white dark:bg-slate-700 text-blue-600 dark:text-blue-400 shadow-sm'
              : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'
            }`}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          <label htmlFor="cnpj" className={`block text-sm font-bold mb-1 ${cnpjError ? 'text-red-600 dark:text-red-400' : 'text-blue-600 dark:text-blue-400'}`}>
            {data.isGroupAnalysis ? 'CNPJ da Holding (Opcional)' : 'CNPJ'} <span className={`${cnpjError ? 'text-red-400' : 'text-blue-400 dark:text-blue-300'} font-normal text-xs`}>(Busca Automática)</span>
          </label>
        ...
        <div>
          <label htmlFor="companyName" className="block text-sm font-bold text-blue-600 dark:text-blue-400 mb-1">
            {data.isGroupAnalysis ? 'Nome do Grupo Econômico' : 'Empresa / Cliente'}
          </label>
          <input
            type="text"
            id="companyName"
